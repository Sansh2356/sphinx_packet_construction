...
pub fn compute_shared_secrets(
    ...
    for hop in hops.iter() {
        ...
        let intermediate_pub_key = PublicKey::from_byte_array_compressed(intermediate_hop_pubkey_bytes)
...
pub fn generate_filler(hops: &Vec<Hops>, secrets: &Vec<[u8; 32]>) -> Vec<u8> {
    ...
    for (hop_num, _hop) in hops[0..hops.len() - 1].iter().enumerate() {
        ...
        let curr_hop_secret = secrets.get(hop_num)
            .expect("Missing hop secret")
            .clone();
        ...
        let full_cipher_stream_xor_slice = &full_cipher_stream
            [stream_slice_start as usize..(MIX_HEADER_SIZE as u32 + curr_hop_payload_size) as usize];
...
pub fn compute_next_hmac(
    ...
    let mut mac = HmacSha256::new_from_slice(mu_key)
        .expect("Failed to create HMAC");
    mac.update(&serialized_input);
    
    mac.finalize().into_bytes().try_into()
